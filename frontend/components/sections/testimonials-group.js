    ...
    <section className="text-center text-lg bg-default pt-12 pb-16">
      ...
      <div className="max-w-5xl w-8/12 sm:w-8/12 bg-white shadow-md sm:shadow-xl mx-auto flex flex-col sm:flex-row mt-10 text-left">
        ...
        <div className="px-4 py-4 sm:px-12 sm:pt-12 sm:pb-4 flex flex-col justify-between">
          ...
          >
            <span className="uppercase tracking-wide text-blue-700 hover:underline  with-arrow sm:self-end mt-6 sm:mt-0">Read story</span>
          </CustomLink>
        </div>
      </div>
      */}
      ...
      */}
        ...
        <div className="flex flex-row flex-wrap container max-w-4xl items-center gap-6 sm:gap-16 justify-center mt-10 px-6 sm:px-0">
          {data.logos.map((logo) => (
            <span key={logo.id} className="bg-white px-3 md:px-5 py-1 rounded-xl">
              <NextImage width="100" height="60" media={logo.logo} className="logo-wrap opacity-80 hover:opacity-100" />
            </span>
          ))}
        </div>
      </div>
